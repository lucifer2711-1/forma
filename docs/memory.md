...
| 2026-09-20 | **Guided six-side walk, one named side at a time** | Walking a free circle is the slowest way to scan: the user holds a whole lap in their head, re-covers finished sides, and decides alone when to stop. Six named steps with a visible end is faster and needs no judgement (`features/capture/coverage/capture_steps.dart`, pure and unit-tested) |
| 2026-09-20 | **The shutter is `requestImageCapture()`, gated on `canRequestImageCapture`** | The session is the only thing that knows whether a frame can be taken; anything else is a tap that silently vanishes (gotcha 37) |
| 2026-09-20 | **Underside via `beginNewScanPassAfterFlip()`, counted from the flip** | The only supported way to get a real bottom, and after a flip the underside is no longer geometrically "down" (gotcha 38) |
...
37. **`ObjectCaptureSession` has manual capture, and we spent sessions not
    knowing it.** The three APIs that make a guided "one side at a time" walk
    possible are all **iOS 17.0** (not 18, unlike `isAutoCaptureEnabled` —
    check each one, they differ):
    - `requestImageCapture()` — captures **one frame where the phone is aimed
      right now**. Silently ignored unless state is `.capturing` **and**
      `canRequestImageCapture` is true, so the button must be driven by that
      property rather than by the phase, or a tap vanishes with no error.
    - `canRequestImageCapture` (+ `canRequestImageCaptureUpdates`) — the
      session's own answer to "can a frame be taken now?", which is what the
      shutter greys out on.
    - `beginNewScanPass()` / `beginNewScanPassAfterFlip()` — a **second pass**
      on the object, the latter meant to be called after the user turns it
      over so the bottom is showing. It returns the session to `.ready` for a
      new bounding box, and the new images go to the **same output directory**
      (Apple: reconstruction stitches the passes automatically). This is the
      supported way to get a real underside; there is no underside to walk to.
38. **After a flip, the underside is not "down" any more.** The direction
    recorder's frame is gravity-aligned, and a flip physically turns the
    object over — its bottom now faces the user, so the direction it would be
    shot from is horizontal and the `(0, 0, -1)` target can never be met again.
    So the underside is counted from **the flip itself** (`flipKeptCount`: any
    frame kept after the second pass started), because the pass exists for
    exactly one side and only ever runs because the user asked for it.
39. **Two walk helpers calling each other overflowed the stack.**
    `CaptureStepPlan.statusOf` asked `currentId` for the current side and
    `currentId` asked `statusOf` for each side's state — an infinite loop that
    blew up the whole plan the moment any widget read a chip's status. Fixed by
    a private `_isDone(id)` that both are built from: **when one derived value
    is defined in terms of another, make sure only one of them depends on the
    other.**
...
- MethodChannel `com.forma.app/native`: isScanSupported, hasLiDAR,
  ...
  setTorch{enabled}, setScanProfile{profile},
  requestImageCapture{scanId}, beginPassAfterFlip{scanId},
  deleteScan{scanId}.
- EventChannel `com.forma.app/capture_events`: payloads `{"type": …,
  ...
  canCapture} / scan_direction{x,y,z,kept} / model_zoom (double) /
...
| 23 | 2026-09-20 | User: "add feature to scan each side by just clicking on it — show right side, left and all of it, then capture them one by one". Read Apple's `ObjectCaptureSession` page instead of assuming, and found three APIs every previous session had missed (gotcha 37): **`requestImageCapture()`** (a real manual frame at the current position), **`canRequestImageCapture`** (the session's own answer to whether a frame can be taken *now*), and **`beginNewScanPassAfterFlip()`** (Apple's supported two-pass flow for the underside — the second pass's images land in the same directory and reconstruction stitches them automatically). All three are **iOS 17.0**, unlike `isAutoCaptureEnabled`/`shouldPlayHaptics` which really are 18.0. Shipped: a **guided six-side walk** (`capture_steps.dart`, pure and unit-tested) that anchors "front" on the first level frame and derives right/back/left by rotating the bearing — the +90° sign for "right" is pinned by a test with the concrete coordinates, because this project has shipped inverted guidance before; a named-side instruction for each step in the user's own frame of reference ("step to your right", never "the object's right"); **tappable side chips** so the user can ask for any side next; a real **shutter** that is enabled exactly when the session says it can take a frame; a **flip-and-scan** action for the underside that counts the underside from the flip itself (gotcha 38); and **skip**, because a checklist that cannot be satisfied is worse than none (gotcha 35). Two walk helpers calling each other overflowed the stack on the first run (gotcha 39) — caught by the unit tests, not by CI. 81 tests green (12 new); build +18 | Install +18 → confirm `v1.0.0+18`; scan a small object: the hint should ask for the **front**, then the chips should tick as frames land and ask for the **right**, then **back**, **left**, **top**, and finally offer **Flip and scan** for the underside. Report whether the shutter greys out when it should |
